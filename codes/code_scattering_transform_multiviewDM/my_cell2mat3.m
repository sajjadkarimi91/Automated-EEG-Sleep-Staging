function [X,Y,Z] = my_cell2mat3(cell_feature,flag_2D)
...
if flag_2D == 0
    ...
    Z = zeros(Nx,Ny,total_Nz);
    Z(:,:,1:Nz(1)) = cell_feature{1,3};
    for k = 2:size(cell_feature,1)
        temp = cell_feature{k,3};
        Z(:,:,cum_Nz(k-1)+1:cum_Nz(k)) =  temp;
    end
